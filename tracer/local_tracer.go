...
var _ vm.Tracer = (*LocalTracer)(nil)
...
func (t *LocalTracer) OnBlockStart(block *ptypes.EvmHeader) {
...
func (t *LocalTracer) CaptureEnd(output []byte, gasUsed uint64, _ time.Duration, err error) {
	if t.callTracer != nil {
...
func (t *LocalTracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, depth int, err error) {
	if t.callTracer != nil {
...
func (t *LocalTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	if t.callTracer != nil {
